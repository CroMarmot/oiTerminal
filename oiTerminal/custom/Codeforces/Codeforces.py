...
class Codeforces(BaseOj):
    ...
    def problem(self, problem_id: str) -> ParseProblemResult:
        ...
        problem.url = url
        problem.oj: str = Codeforces.__name__
        problem.file_path: str = self.pid2file_path(problem_id)
        return problem
    ...
    def login_website(self, account: Account) -> int:
        ...
        if self._is_login():
            account.cookie = self.http_util.cookies.get_dict()  # outer can get and save cookie from user
            self.account = account
            return 60 * 20
        else:
            return -60
