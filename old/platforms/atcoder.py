...
"""
  ...
  def problem_parse(self, problem: Problem, response: str):
    ...
    if match_groups:
      problem.title = ' - '.join(
          match_groups.get_text().split(' - ')[1:])

    match_groups = match_groups.find_next_sibling('p')
    # Time Limit: 2 sec / Memory Limit: 1024 MB
    if match_groups:
      result = re.match('^Time Limit: (.*)\/ Memory Limit: (.*)$',
                        match_groups.get_text().strip(' \n\t'))
      try:
    ...
    if match_groups and isinstance(match_groups, element.Tag):
      for child in match_groups.children:
        if isinstance(child, element.Tag) and child.get('class') and set(child['class']).intersection(
          ...
          problem.html += str(HtmlTag.update_tag(child,
                              self._static_prefix))
        else:
          problem.html += str(HtmlTag.update_tag(child,
                              self._static_prefix))
    ...
    in_test = re.findall('Sample Input \d</h3><pre>(.*?)</pre>',
                         lang_en_str, flags=re.MULTILINE | re.DOTALL)
    out_test = re.findall('Sample Output \d</h3><pre>(.*?)</pre>',
                          lang_en_str, flags=re.MULTILINE | re.DOTALL)
...
class AtCoder(Base):
  ...
  def login_website(self, account: Account) -> int:
    ...
    if self._is_login():
      ...
      account.cookie = self._req.cookies.get_dict()
  ...
  def submit_code(self, pid: str, language: str, code: str) -> bool:
    ...
    res = self._req.get('https://atcoder.jp/contests/' +
                        result.group(1) + '/submit')
    if res is None:
      raise Exception(
          f"submit_code: cannot open problem,pid={pid},language={language}")
    soup = BeautifulSoup(res.text, 'lxml')
    csrf_token = soup.find('input', attrs={'name': 'csrf_token'})['value']
    r = re.search('<option value="([^"]*?)">' +
                  result.group(2), str(soup), re.DOTALL)
    post_data = {
        'csrf_token': csrf_token,
  ...
  def get_result(self, pid) -> Result:
    ...
    r = re.search('<option value="([^"]*?)">' +
                  result.group(2), str(soup), re.DOTALL)
    request_url = 'https://atcoder.jp/contests/' + \
        result.group(1) + '/submissions/me?f.Task=' + r.group(1)
    ...
    url = 'https://atcoder.jp/contests/' + \
        r.group(1) + '/submissions/me/status/json?sids[]=' + r.group(2)
    print(url)
  ...
  def get_language(self) -> LangKV:
    ...
    if res.text:
      soup = BeautifulSoup(res.text, 'lxml')
      tags = soup.find(
          'div', attrs={'id': 'select-lang-practice_1'}).find('select')
      if tags:
